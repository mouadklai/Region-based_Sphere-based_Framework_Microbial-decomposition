...
Z_profiles_100 = np.loadtxt('Data/LBIOS/table_DOC_profileZ_BD12_Sw80_15000.txt')[:,0]

#Regions and adjacency
regions = np.loadtxt("Data/Drainage_ouverture/80%/regions.txt")
voisins = np.loadtxt("Data/Drainage_ouverture/80%/voisins.txt")

regions_minmax_plans = np.loadtxt("Data/Drainage_ouverture/80%/region_z_limits.txt")
indices = np.union1d(np.loadtxt("Data/Drainage_ouverture/80%/new_z_plans/z1.txt"),np.loadtxt("Data/Drainage_ouverture/80%/new_z_plans/z2.txt"))
...
figure.suptitle(f"LBIOS vs MOSAIC_regions diffusion for 80% saturation Dc={Dc}")
plt.show()