...
def Asynchronous_Transformation(regions,initialDistribution,deltat,rho, mu, rho_m,vfom,vsom,vdom,kab):
    N = regions.shape[0]
    X = np.empty_like(initialDistribution)
    for i in nb.prange(N):
        # Now we are in the ball Let's initialize the variables and set some parameters
        volume = regions[i,3] # radius of the ball
...
def Simulate_explicit(BallsSet,Theta,initialDistribution,DeltaT,Dc,deltat,dt,rho,mu,rho_m,vfom,vsom,vdom,kab):
    ...
    print("Performing asynchrone Transformation with",
          deltat*60*60*24*1000,
          " ms and explicit diffusion with time step= ",
          dt*60*60*24*1000,
          "ms and a Diffusion coefficient Dc=",
            Dc,
          'for',
          DeltaT,
          "days,\n")
...
def Implicit_scheme_regions(voisins,regions,Dc,deltat):
    N= regions.shape[0]
...
def Simulate_implicit(A,M_inv,regions,initialDistribution,DeltaT,Dc,deltat,dt,rho,mu,rho_m,vfom,vsom,vdom,kab):
    TotalDistribution = np.sum(initialDistribution,axis=0)*100/np.sum(initialDistribution)
    history = [TotalDistribution]
    X = initialDistribution.copy()
    ...
    with tqdm(total=int(DeltaT/deltat)) as pbar:
        ...
        for _ in  nb.prange(int(DeltaT/deltat)):
            #transformation
            X = Asynchronous_Transformation(regions,X,deltat,rho,mu,rho_m,vfom,vsom,vdom,kab) 
            #Diffusion 
...
def fill_Balls_with_MB_Distributeoverthenearest_regions(regions,voxels_spots,DOM_mass):
    output = np.zeros((regions.shape[0],5))
    totalVolume = np.sum(regions[:,3])
    for spot in voxels_spots:
        index = (spot[0] - regions[:,0])**2 + (spot[1] - regions[:,1])**2 + (spot[2] - regions[:,2])**2 
        index = np.where(index == index.min())[0] 
        V = np.sum(regions[index,3])
        for j in index:
            output[j,0] += regions[j,3]/V * spot[3]
    for i in  nb.prange(regions.shape[0]):
        v = regions[i,3]
        output[i,1] = DOM_mass*v/totalVolume
    return output

def fill_Balls_with_MB_Distributeoverthenearest_balls(BallsSet,voxels_spots,DOM_mass):
...
def compare_bio(history_file1,history_file2,discription1,discription2,simulationTime,comparison_experience):
    history1 = np.loadtxt(history_file1)
    history2 = np.loadtxt(history_file2)
    dt1 = simulationTime / history1.shape[0]
    dt2 = simulationTime / history2.shape[0]
    print("time step in file 1 : " , dt1)
    print("time step in file 2 : " , dt2)
    x1 =  np.linspace(0,history1.shape[0]*dt1,history1.shape[0])
    x2 =  np.linspace(0,history2.shape[0]*dt2,history2.shape[0])
    figure, ax = plt.subplots()
    ax.plot(x1, history1[...,0],'b-.',  label=discription1+' MB')
    ax.plot(x1, history1[...,1],'g-.',  label=discription1+' DOM')
    ax.plot(x1, history1[...,2],'c-.',  label=discription1+' SOM')
    ax.plot(x1, history1[...,4],'r-.',  label=discription1+' CO2')
    ax.plot(x2, history2[...,0],'b',  label=discription2+' MB')
    ax.plot(x2, history2[...,1],'g',  label=discription2+' DOM')
    ax.plot(x2, history2[...,2],'c',  label=discription2+' SOM')
    ax.plot(x2, history2[...,4],'r',  label=discription2+' CO2')
    figure.patch.set_facecolor('white')
    figure.suptitle(comparison_experience,fontdict={'size':15})
    ...
    figure.savefig("Plots and Results/"+comparison_experience+'.png')
    plt.show()




def Implicit_scheme_balls(Adjacency,BallsSet,Dc,deltat):
    N= BallsSet.shape[0]
    R,C,V = [i for i in nb.prange(N)],[i for i in nb.prange(N)],[1 for i in nb.prange(N)]
    Volume = sparse.csr_matrix((np.array(V), (np.array(R), np.array(C))), shape=(N,N))
    Volume_inv = sparse.csr_matrix((1/np.array(V), (R, C)), shape=(N,N))
